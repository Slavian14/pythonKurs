...
player_img = pygame.image.load(os.path.join(img_folder, 'bhort_1.png'))
...
def show_go_screen():
    ...
    draw_text(screen, "Привет игроманы!", 52, WIDTH / 2, HEIGHT / 4)
    draw_text(screen, "Хотите начать? Нажмите пробел))", 22,
              WIDTH / 2, HEIGHT / 2)
    draw_text(screen, "Ну нажмите:)", 18, WIDTH / 2, HEIGHT * 3 / 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.power >= 1.4 and pygame.time.get_ticks() - self.power_time > POWERUP_TIME:
            self.power -= 1
            self.power_time = pygame.time.get_ticks()
...
background = pygame.image.load(path.join(img_folder, "starBackground.png")).convert()
background_rect = background.get_rect()
player_img = pygame.image.load(path.join(img_folder, "bhort_1.png"))
player_mini_img = pygame.transform.scale(player_img, (25, 19))
player_mini_img.set_colorkey(BLACK)
meteor_img = pygame.image.load(path.join(img_folder, "meteorSmall.png")).convert()
